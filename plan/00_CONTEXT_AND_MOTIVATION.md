...
> **Do not use this as evidence against Ray Serve.** An unfinished internal experiment says nothing about the quality of a mature framework, and [`12_REFERENCE_CODE.md`](12_REFERENCE_CODE.md) §8 shows the actual defect was a *caller* force-flushing its own batch — a misuse of a batching pattern, not a fault in Ray. The Ray Serve question is answered properly, on its own merits, in [`15_RAY_SERVE_EVALUATION.md`](15_RAY_SERVE_EVALUATION.md) and [ADR-0003](adr/0003-ray-serve-not-adopted.md). Note also that "Ray is heavy" is asserted here and **nowhere measured**; treat it as a hypothesis.
...
|---|---|---|
| **llama-swap** | YAML-configured proxy that starts/stops upstreams on demand, with TTL, groups, and an OpenAI-compatible front door. Single Go binary. **Upstreams may be processes *or containers*** — `cmd` + `cmdStop` with `docker run` is a documented, headline feature. | **Two roles.** (1) It is our *sibling service*: LLM serving happens there, not here, which is why tool-swap has no OpenAI layer. (2) It is the direct model for our router semantics: config shape, `ttl`, `groups`, `macros`, `/upstream/{tool}` proxying, the `/ui` status page. **Read [`third-party-docs/llama-swap/`](third-party-docs/llama-swap/INDEX.md) — captured at v249 — before designing the config.** ⚠️ **This row used to say "it manages processes not containers". That was stale**: containers are supported, and upstream *recommends* them for Python inference servers to get *"clean environment isolation"* — **which is D2's own reason**. The real difference is narrower and is the one that matters: **they route by `body.model` on generative-API paths; our tools are addressed by path with typed bodies, and their config cannot express that.** |
...
| **BentoML** | What R8 already used: Python-native services with `batchable` APIs, `max_batch_size` / `max_latency_ms`, an adaptive dispatcher, `/healthz` `/readyz` and Prometheus metrics. | **ADOPTED as the in-container runtime** (**D14**, resolving Q1), wrapped behind our own contract so that the author's `handler.py` never imports it and the router never sees it. We therefore write no batcher at all. Its constrained dependencies are the accepted risk, instrumented by a spike and a CI resolution gate ([`05_RUNTIME_AND_BATCHING.md`](05_RUNTIME_AND_BATCHING.md) §1). ⚠️ **The binding constraints are `cattrs`, a beta-pinned OpenTelemetry family and `fsspec` — not the "pydantic, starlette, click" named here for months**, two of which are lower bounds that cannot conflict ([`third-party-docs/bentoml/dependency-constraints.md`](third-party-docs/bentoml/dependency-constraints.md)). |
| **Ray Serve** | Autoscaling, `@serve.batch` dynamic batching, per-deployment resources, **`@serve.multiplexed`** — a bounded LRU cache of models in a live replica, very nearly our own central primitive — **and `runtime_env.image_uri`, which runs each Serve application in its own container image**. | **The closest match of any candidate**, evaluated in full in [`15_RAY_SERVE_EVALUATION.md`](15_RAY_SERVE_EVALUATION.md). **Note: Ray *can* give each tool its own image, so D2 is satisfiable on Ray** — an earlier claim here to the contrary was wrong. Not adopted *for now* ([ADR-0003](adr/0003-ray-serve-not-adopted.md), provisional) because displacement lives inside a replica while isolation lives around it, so a *containerised* tool cannot be cheaply displaced (**D25**), there is no alive-but-unloaded state (**D9**), and `image_uri` is experimental and locks every image to the cluster's exact Ray and Python patch version. Ray *inside* a tool image remains permitted. |
