...
Two facts from the gate shape the whole design, and **[ADR-0002](adr/0002-shared-node-soft-unload.md)** records them:

1. **Preemption is required.** A request for tool B displaces an idle incumbent immediately rather than waiting out its TTL (**D25**). This is what ruled out Kubernetes.
2. **We share the node.** The contended resource is node VRAM, not a slot in our own table — so **soft unload is the default reclamation mechanism**, not the phase-2 optimisation **D9** originally made it. Hard stop is the backstop. A reload can fail because a *neighbour* took the memory, and that must never be recorded as a tool failure (**D28**).
...
| | **D9** *(amended)* | **Both TTLs ship in v1; soft unload is the default reclamation mechanism**, hard stop the backstop. Build hard first in TDD order. | The node is a **shared DGX**, so the contended resource is node VRAM rather than our own group slot — which reverses the original reasoning for deferring soft TTL. See [ADR-0002](adr/0002-shared-node-soft-unload.md). |
| | **D25** | **A request preempts an idle incumbent** rather than waiting for its TTL. | Stated as a requirement; it is also the decision that ruled out Kubernetes, which allocates rather than preempts. Soft unload is what makes displacement cheap. |
| | **D26** | **`unload()` is mandatory for GPU tools**, and preflight stage 8 verifying real VRAM release is a hard `FAIL`. A handler that cannot release is classified *hard-stop-only*, not rejected. | Soft unload is now how VRAM returns to a node we share; a handler that only *appears* to release degrades the whole box silently. |
...
| | **D28** | **A reload that fails for lack of VRAM is not a tool failure.** Back to `IDLE_SOFT`, no failure-budget increment, 503 `vram_unavailable`. | On a shared node a neighbour can take memory we released. Marking our tool broken for someone else's usage is wrong and misleads whoever is on call. |
...
| **TTL (hard)** | Idle seconds after which the container is **stopped**. Frees VRAM, RAM and the group slot. |
| **TTL (soft)** | Idle seconds after which the container is asked to **release its weights** but stays alive. Cheap reload. Phase 2. |
...
| | [`13_OPEN_QUESTIONS.md`](13_OPEN_QUESTIONS.md) | The decision log (D1–D24, with reasoning), the few genuinely open questions, parked ideas, and the guardrails that must not be "improved" away. |
...
| | [`adr/`](adr/README.md) | **Decisions taken after the plan was written.** [ADR-0001](adr/0001-build-our-own-router.md) closes the router gate; [ADR-0002](adr/0002-shared-node-soft-unload.md) records the shared-DGX reversal that moved soft unload into v1. |
