...
### A fourth candidate, challenged later: Ray Serve — spiked, measured, decided

*"I got challenged that we were reimplementing Ray Serve — why not use this framework?"* Ray Serve had only ever received a one-paragraph dismissal and was never spiked, so the challenge was fair. **Spike E answered it with measurement** — a nine-step evaluation run on a GPU host — and **[ADR-0003](adr/0003-ray-serve-not-adopted.md) was re-based onto the result** (**D29**). **Start from the spike's entry point** ([`spike-e-ray-native/README.md`](../spike-e-ray-native/README.md)): what the spike asked, what it found, what was decided, and which document to read for which purpose (the evidence ledger, the analysis of record, the ADR, the frozen protocol).

**The decision, with its scope qualifier — it must travel together:** *Ray Serve is not the router for tool-swap's v1 single-host model-switching workload.* It is not a verdict on Ray in general. The spike's capability results ran **against** the ADR — Ray passed most of the protocol, including per-tool container images, real GPU displacement, and 20/20 request-driven alternations with zero errors. What decided it is **one measured failure on our core path**: with a request arriving *during* the swap — tool-swap's actual trigger — a large share of cycles took ~100 s of dead time inside Ray before the container was even launched, versus 3.9–6.9 s for plain podman; the mechanism is deliberately unnamed. Where Ray *is* the right tool is stated in [`plans/ray-adoption-analysis.md`](../plans/ray-adoption-analysis.md) §0′ (revision 3, the analysis of record); the pre-spike evaluation [`15_RAY_SERVE_EVALUATION.md`](15_RAY_SERVE_EVALUATION.md) (v3) is preserved as the error trail, and its three capability concessions stand un-argued. **Spike D is retired**, because [ADR-0004](adr/0004-hard-stop-only-in-v1.md) removed the one question it existed to answer.
...
| | **D14** | **The in-container runtime is built on BentoML**, wrapped behind our own contract. Batching is the cumbersome, correctness-critical part, and BentoML's adaptive dispatcher already does it well — better than a fixed policy of our own — and the originating system ran it successfully. Its constrained dependencies are accepted *until a concrete tool proves an unresolvable conflict*, with a spike and a CI resolution gate to find out. A `RuntimeBackend` seam keeps the alternative open as **specified architecture with no v1 code**. | The author never sees BentoML; the handler stays framework-free. See [`05_RUNTIME_AND_BATCHING.md`](05_RUNTIME_AND_BATCHING.md) §1–§2. **The constraints that actually bind are `cattrs`, the OpenTelemetry family and `fsspec` — not the "pydantic, starlette, click" this plan named for months** ([`third-party-docs/bentoml/dependency-constraints.md`](third-party-docs/bentoml/dependency-constraints.md)). |
| | **D15** *(re-scoped)* | **One uniform calling convention: every handler takes and returns a list, always.** Per-request knobs are **fields of the item**, so they are per-item by construction; `params:` narrows to values that change what the batched forward pass computes. `scalar_inputs` and the per-input `batchable:` flag are gone. | Originally a *ban* on non-batchable inputs, because the dispatcher cannot separate a batch by differing arguments and would answer one request with another's threshold — silently. BentoML documents a per-item remedy, so the hazard is now **unrepresentable rather than forbidden**, and the authoring surface loses two concepts. See **[ADR-0005](adr/0005-one-uniform-batched-calling-convention.md)**. |
...
| | **D29** *(settled, with triggers)* | **Ray Serve is not the router for tool-swap's v1 single-host model-switching workload; Ray *inside* a tool image is permitted.** | **Spike E measured it** and [ADR-0003](adr/0003-ray-serve-not-adopted.md) was re-based onto the result (2026-09-14). The capability case ran against us — Ray passed most of the protocol, including real GPU displacement and 20/20 request-driven alternations. The deciding ground is measured, not argued: ~93 s of pre-container dead time on ~40% of request-triggered swaps (our core path), mechanism deliberately unnamed, against 3.9–6.9 s for plain podman. Not a verdict on Ray in general — where Ray *is* the right tool is in [`plans/ray-adoption-analysis.md`](../plans/ray-adoption-analysis.md) §0′. **Spike D is retired**. Entry point: [`spike-e-ray-native/README.md`](../spike-e-ray-native/README.md). |
...
| | [`15_RAY_SERVE_EVALUATION.md`](15_RAY_SERVE_EVALUATION.md) | The pre-spike Ray Serve evaluation (**v3**). The decision is now re-based on measurement — [ADR-0003](adr/0003-ray-serve-not-adopted.md) and the spike's entry point ([`spike-e-ray-native/README.md`](../spike-e-ray-native/README.md)) are the current reading; the full post-spike argument is in [`plans/ray-adoption-analysis.md`](../plans/ray-adoption-analysis.md) (rev 3, which supersedes this file as the place to read the argument). The three-role separation and the three capability concessions still stand. Read the v3 text as the error trail — its grounds (experimental APIs, version lockstep) were demoted, not confirmed, by the spike. |
...
| | [`17_LLAMA_SWAP_PHILOSOPHY.md`](17_LLAMA_SWAP_PHILOSOPHY.md) | **"Do what llama-swap does, but for generic ML models."** The philosophy we are borrowing, a behaviour-by-behaviour mapping of every llama-swap primitive to ours, what we refuse and why it all traces to one fact, the five things that genuinely change when the upstream is an arbitrary model rather than an LLM, and **five gaps llama-swap has already solved that this plan has not noticed** (one of them due before M5). Grounded in [`third-party-docs/llama-swap/`](third-party-docs/llama-swap/INDEX.md), whose findings correct two stale claims elsewhere in this plan. |
| | [`adr/`](adr/README.md) | **Decisions taken after the plan was written.** [ADR-0001](adr/0001-build-our-own-router.md) closes the router gate; [ADR-0002](adr/0002-shared-node-soft-unload.md) records the shared-DGX reading that moved soft unload into v1; [ADR-0003](adr/0003-ray-serve-not-adopted.md) answers the Ray Serve challenge; **[ADR-0004](adr/0004-hard-stop-only-in-v1.md) reverses ADR-0002 and takes soft unload back out of v1** — read it before implementing anything in the lifecycle layer; **[ADR-0005](adr/0005-one-uniform-batched-calling-convention.md) re-scopes D15 into one uniform calling convention** — read it before implementing the runtime or the schema compiler. |
