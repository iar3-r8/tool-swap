...
| `tswap logs <tool> [-f] [--tail N] [--since 10m]` | That tool's logs. **Works for a `STOPPED` tool**, returning its retained history rather than an error — a TTL-stop must not erase the evidence you came to read. llama-swap's own rewrite shipped a 400 in exactly this case. |
...
### 2.1 What reaches stdout: `log_output`

Borrowed from llama-swap's `logToStdout`, which states the same choice more cleanly than a set of per-tool toggles:

| `router.log_output` | stdout carries |
|---|---|
| `router` *(default)* | Lines generated by tool-swap itself |
| `tool` | A copy of the tool containers' output |
| `both` | Both, interleaved |
| `none` | Nothing (files only) |

**This setting is *only* about stdout.** Rule 1 below is unaffected: logs are **always persisted to files** whatever it says, so `none` never means "lose the diagnostics".

> ⚠️ **Test that this setting actually does something.** In llama-swap's router rewrite, `logToStdout` was *"ignored"* — hardcoded to stdout — and the combined `/logs` history was *"always empty"*, in code that had passed a completed review. **Nothing 500s when logging silently breaks**, and **R1** is precisely *"simple to launch the service and check its logs and status"* — so this is a requirement failure, not a cosmetic one ([`10_TESTING_STRATEGY.md`](10_TESTING_STRATEGY.md) §6.1).
...
| 503 `TOOL_UNAVAILABLE` (`reason: vram_unavailable`) | **Another tenant on the shared DGX holds the memory.** The tool is fine and stays `STOPPED` | Nothing to fix in the tool. Retry; `nvidia-smi` shows who holds it. If persistent, `keep_warm: true` on the affected tool, or fewer tools competing |
| 503 with a **saturation** reason on a tool that is `READY` | The tool cannot keep up; the dispatcher missed its latency budget | Raise `max_latency_ms`, lower `max_batch_size`, or set `max_concurrent` to shed load at the router instead ([`06_LIFECYCLE_TTL_AND_SCHEDULING.md`](06_LIFECYCLE_TTL_AND_SCHEDULING.md) §10). **Not a cold start and not a broken tool** |
| A tool works but `tswap status` shows VRAM never returning after idle | The container is not being stopped — check `ttl`, `keep_warm` and whether `inflight` is stuck above zero | VRAM is released by stopping the container, so investigate the TTL path rather than the handler. *(`unload()` is advisory and is not on the reclamation path — [ADR-0004](adr/0004-hard-stop-only-in-v1.md).)* |
