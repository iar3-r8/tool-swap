...
| [`docker/`](docker/INDEX.md) | https://docker-py.readthedocs.io (interface read from the installed package source instead) | docker-py 7.2.0 | **Working reference material for M2a** — the docker-py interface that [`plans/m2a-container-backend-seam.md`](../../plans/m2a-container-backend-seam.md) §3 made a blocking pre-condition for behaviours 14–26, recorded from the installed 7.2.0 source so kwarg names, exception classes and filter shapes are cited, not recalled |
