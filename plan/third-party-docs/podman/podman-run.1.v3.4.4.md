<!--
Source: https://raw.githubusercontent.com/containers/podman/v3.4.4/docs/source/markdown/podman-run.1.md
Upstream version: podman v3.4.4 (host version for the D17 spike) — captured 2026-08-26.
Relevance: answers whether `podman run tool_torch:spike` resolves a locally built short-name
image from local storage or attempts registry resolution (see `--pull`, below).
Complete capture, unedited; only the HTML anchor markup of the raw markdown was left as-is.
-->
...
- **`--pull` defaults to `missing`**: "attempt to pull the latest image from the
  registries listed in registries.conf **if a local image does not exist**."
  So a locally built `tool_torch:spike` is found in local image storage and
  **no registry (and no short-name) resolution is attempted** — the local
  lookup short-circuits before the name ever reaches the short-name /
  unqualified-search machinery.
- The DESCRIPTION also states the same precedence: "If the _image_ is not
  already loaded then **podman run** will pull the _image_ ... before it
  starts the container from that image."
- **Consequence for D17:** the fixture images at run time
  (`tool_torch:spike` / `tool_tf:spike` passed to `podman run` by Ray's
  `image_uri` runtime env) are **not** affected by the host's missing
  `unqualified-search-registries`, as long as the images were just built
  locally on that host with `--pull` left at its default. The
  build-time failure (the base image in `FROM`) is a different path:
  building a stage requires pulling the base, and an absent local image
  *does* reach short-name resolution and fail there.
- **Residual risk (not a defect per the docs):** `--pull missing` triggers a
  pull when the local image is *absent* (e.g. after `podman system prune -a`
  or on a worker node that did not build the image). In that case the
  short name goes through `registries.conf` resolution and, on a host with
  no `unqualified-search-registries` and no alias, fails exactly like the
  base image did. `localhost/tool_torch:spike` would make that path
  deterministic (see [short-name-resolution.md](short-name-resolution.md)).
  Not implemented in D17 — recommendation only.
