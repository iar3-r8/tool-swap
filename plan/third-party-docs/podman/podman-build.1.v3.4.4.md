<!--
Source: https://raw.githubusercontent.com/containers/podman/v3.4.4/docs/source/markdown/podman-build.1.md
Upstream version: podman v3.4.4 (host builder for the D17/D19 spikes) — captured 2026-08-26.
Relevance: answers whether `podman build` supports `COPY --chown=<user>` with a bare
username (no `:group`). This man page delegates Containerfile-instruction semantics to
the vendored buildah code (see "NOTE: podman build uses code sourced from the buildah
project", and podman v3.4.4's go.mod pins github.com/containers/buildah v1.23.1);
buildah v1.23.1 accepts `--chown=<uid:gid>` on COPY and resolves a bare username via
/etc/passwd (chrootuser.GetUser) — see plan/third-party-docs/podman/INDEX.md.
