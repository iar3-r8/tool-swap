<!--
Source: https://raw.githubusercontent.com/containers/image/v5.17.0/docs/containers-registries.conf.5.md
Upstream version: containers/image v5.17.0 — the exact version vendored by
podman v3.4.4 (see https://raw.githubusercontent.com/containers/podman/v3.4.4/go.mod,
line: `github.com/containers/image/v5 v5.17.0`) — captured 2026-08-26.
Relevance: the authoritative `registries.conf` reference for the host's
short-name resolution rules (unqualified-search-registries, aliases,
short-name-mode) behind the D17 base-image build failure.
Complete capture, unedited.
-->
...
- `unqualified-search-registries`: "An array of _host_[`:`_port_] registries
  to try when pulling an unqualified image, in order." If the array is
  undefined (as on the D17 host), there is no fallback — an unqualified
  (short) name that has no alias cannot be resolved, and pull fails.
- Short-name aliases (`[aliases]` table) can map a short name to a
  fully-qualified reference, but "name" **cannot include a registry domain
  or refer to localhost** — so aliases cannot reference
  `localhost/tool_torch:spike`-style names; a locally built image is only
  addressed via its local-storage name or an explicit `localhost/...`
  registry reference.
- `short-name-mode` (default `permissive` when unset): with more than one
  unqualified-search registry and no TTY, `enforcing` errors; `permissive`
  falls back to trying all listed registries. With *zero* entries, neither
  mode can help — the failure is deterministic.
- The closing note: "We recommend always using fully qualified image names
  including the registry server (full dns name), namespace, image name, and
  tag." — the man page itself prescribes what D17 applies to `RAY_BASE_TAG`.
- "Normalization of docker.io references": `docker.io/rayproject/ray` is
  *not* rewritten to `/library` (that normalization only applies to the
  implied one-component name), so `docker.io/rayproject/ray:2.57.0-py311-gpu`
  resolves to the same remote image the short name would have, once a
