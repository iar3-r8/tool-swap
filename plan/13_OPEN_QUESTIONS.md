...
**The counter-argument, and the stance taken:** BentoML brings constrained dependencies into every tool image, in tension with **D2**'s per-tool dependency freedom. *A locked dependency is acceptable until a concrete tool demonstrates an unresolvable conflict.* "BentoML constrains pydantic" is not a conflict; "this tool requires pydantic <2 and therefore cannot install alongside BentoML" is. We do not pay the permanent cost of owning an inference server to pre-empt a conflict nobody has hit.

> ⚠️ **Correction (2026-08-13).** This decision named *"pydantic, starlette, click"* as the risk. Checked against the packaging metadata for 1.4.39 ([`third-party-docs/bentoml/dependency-constraints.md`](third-party-docs/bentoml/dependency-constraints.md)), **two of those three cannot conflict at all** — `starlette>=0.24.0` and `click>=7.0` are *lower* bounds — and `pydantic<3` is a wide upper bound. The real two-sided constraints are **`cattrs>=22.1.0,<23.2.0`**, a **seven-package OpenTelemetry family pinned to a beta series**, and `fsspec>=2025.7.0`, none of which appeared anywhere in this plan. **The stance is unchanged and the risk is real; the packages named were the least informative ones.** See [`05_RUNTIME_AND_BATCHING.md`](05_RUNTIME_AND_BATCHING.md) §1.1.
...
**Costs accepted:** batching unit tests move from L1 (`ManualClock`, milliseconds) to L2 (in-process, real time, statistical assertions), because the dispatcher owns its own adaptive timing; and `max_batch_bytes` is not available. *(The third cost — per-request non-batchable inputs becoming illegal — **has been removed** by [ADR-0005](adr/0005-one-uniform-batched-calling-convention.md); see **D15**.)*
...
### D15 *(re-scoped)* — One uniform calling convention: every handler takes and returns a list

> **Superseded in meaning by [ADR-0005](adr/0005-one-uniform-batched-calling-convention.md) (2026-08-13).** What was an authoring *restriction* is now a uniform *convention*. Read the ADR for the full reasoning; this entry records what changed.

**The original rule and why it existed.** BentoML's dispatcher cannot group a batch by non-batchable arguments, so two requests with `threshold=0.5` and `threshold=0.9` would be batched together and one silently answered with the other's parameter: no exception, no log line, and a plausible-looking wrong number returned about a patient. D15 therefore **banned** non-batchable inputs on batched tools, with `tswap validate` hard-erroring, and pushed every per-request knob into a load-time static param.

**What changed.** BentoML documents a second remedy the original decision did not know about — make the batched element a **composite item**, so each item carries its own parameters — and its example is literally ours (`image` + `threshold`). That made the ban over-broad: correct for parameters that change the batched computation, unnecessarily restrictive for post-processing knobs.

**The rule now:** **every handler receives a list of typed items and returns a list of the same length, always.** Per-request knobs are **fields of the item**, so they are per-item by construction and the silent-misattribution hazard **cannot be expressed**. `params:` narrows to values that genuinely change what the batched forward pass computes (input resolution, dtype, a different model head, a weights path).

**The test for `params:` versus `inputs:`** — *does this value change what the batched forward pass computes?* If yes, `params:`. If it only shapes that item's own result, `inputs:`.

**Removed by this re-scoping:** `scalar_inputs`, the per-input `batchable:` flag, the `validate` hard error, the second `/schema` shape, and *"use the `native` backend for per-request knobs"* — which was the seam's stated first use case. **The seam still stands on dependency risk alone**, which is what M3.5 measures.

**What did not change:** the batch-length check and `retry_singly` are **more** load-bearing now that every tool is on one path, not less. Nothing here may be cited to weaken them.

**One premise is unverified:** whether the per-item pattern needs a `bentoml.depends` wrapper Service at all. **M3.5 must settle it before M4** ([ADR-0005](adr/0005-one-uniform-batched-calling-convention.md) §5).
