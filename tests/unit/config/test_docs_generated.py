...
NON_RULE_C_CODES = frozenset(
    {
        ...
        "TSWAP-C001",  # _CODE_SYNTAX (loader.py:60) /
        # _CODE_UNSUPPORTED_VERSION (schema.py:37)
    ...
    for module in (loader, interpolate, schema, resolver, validate):
        for name, value in vars(module).items():
            ...
            if (
                ...
                and _C_CODE_VALUE.fullmatch(value)
            ):
                derived.add(value)
...
def _parse_troubleshooting_rows(text: str) -> dict[str, str]:
    ...
    Splits only the first two cells (code, then cause); the fix is the
    remainder of the line, so a literal pipe inside the fix text stays
    in the fix cell. Verified against all 76 shipped rows: exactly one
    cell carries a pipe — TSWAP-C540's remedy (``ro|rw``) — and no
    cause does, so the bounded split cannot truncate a cause.
    ...
    for line in lines[started + 1 :]:
        ...
        if not stripped.startswith("|"):
            if rows:
                break
            continue
        # maxsplit=2: a raw pipe inside the fix cell (a markdown table
        # cell cannot escape it, so the generator renders it verbatim)
        # must not truncate the cell.
        cells = [cell.strip() for cell in stripped.strip("|").split("|", 2)]
        if len(cells) < 3:
            continue
        code = cells[0].strip("`")
...
def _parse_field_rows(text: str) -> list[tuple[str, str]]:
    """Extract ``(key, default)`` from every per-block field-table row.

    The eight ``| Key | Type | Default | Description |`` tables precede
    the troubleshooting heading; the key cell's backticks are stripped.
    Splits only the first three cells, so a pipe inside the description
    (or any later cell) cannot disturb the Default cell.

    Args:
        text: The full rendered reference (``render_reference()``).

    Returns:
        One ``(key, default)`` pair per field row, in document order.
    """
    lines = text.splitlines()
    try:
        stop = lines.index(_TROUBLESHOOTING_HEADING)
    except ValueError:
        stop = len(lines)
...
def test_the_reference_document_is_committed() -> None:
    ...
    assert REFERENCE.read_text(encoding="utf-8").strip(), f"{REFERENCE.name} is empty"
...
def test_the_generator_script_is_committed() -> None:
    ...
    assert GENERATOR.read_text(encoding="utf-8").strip(), f"{GENERATOR.name} is empty"
...
def test_every_emitted_code_has_a_troubleshooting_row() -> None:
    ...
    assert not missing, "emitted codes with no troubleshooting row: " + ", ".join(
        missing
    )
...
def test_every_troubleshooting_row_is_an_emitted_code() -> None:
    ...
    assert not phantom, "rows for codes the codebase never emits: " + ", ".join(phantom)
...
def test_the_single_required_field_renders_a_dash() -> None:
    """Exactly one field-table row renders the em-dash ``—`` Default.

    ``BuildConfig.context`` is the only required field in the whole
    schema (detected with ``FieldInfo.is_required()``), so the
    "no default" marker appears in exactly one field row's Default
    cell — plan item 3. Scoped to the field tables' Default column:
    em dashes legitimately appear elsewhere in the document (the
    ``GENERATED FILE`` banner, authored descriptions, and the rule
    remedies the fix-column test pins verbatim).
    """
    import scripts.gen_config_reference as gen  # noqa: PLC0415

    text = gen.render_reference()
    dash_defaults = [key for key, default in _parse_field_rows(text) if default == "—"]
    assert dash_defaults == ["context"], (
        f"the em-dash 'no default' marker appears in {dash_defaults!r}; "
        "exactly one required field (BuildConfig.context) is expected"
    )
    # Cross-check the marker against the schema: ``BuildConfig.context``
    # is the only required field, and it must be the one that renders ``—``.
    required = [
        (model.__name__, name)
        for model in _MODELS
        for name, field in model.model_fields.items()
    ...
    assert required == [("BuildConfig", "context")], (
        f"expected BuildConfig.context to be the only required field, found {required}"
    )
